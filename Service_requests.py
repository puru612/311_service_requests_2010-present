...
ser_rq_df=pd.read_csv('/home/labsuser/Assessment/311_Service_Requests_from_2010_to_Present.csv')
...
ser_rq_df=ser_rq_df.dropna(subset=['Latitude','Longitude', 'Location', 'X Coordinate (State Plane)', 'Y Coordinate (State Plane)'], axis=0)
ser_rq_df.shape
...
ser_rq_df=ser_rq_df.dropna(subset=['Incident Address','Incident Zip','Due Date','Street Name','Cross Street 1','Cross Street 2'],axis=0)
ser_rq_df.shape
...
#(Agency, Agency Name, Park Facility Name, Park Borough, Facility Type, School Name, School Number, School Region, School Code, School Phone Number, School Address, School City, School State, School Zip, School Not Found)
ser_rq_df=ser_rq_df.drop(['Agency', 'Agency Name', 'Park Facility Name', 'Park Borough', 'Facility Type', 'School Name', 'School Number', 'School Region', 'School Code', 'School Phone Number', 'School Address', 'School City', 'School State', 'School Zip', 'School Not Found'], axis=1)
ser_rq_df.shape
...
plt.xlabel('Longitude')
plt.ylabel('Latitude')
plt.title('Complaint concentratin across Brooklyn - Hexbin Plot', color='black', backgroundcolor='silver')
plt.show()


# #### Barplot representation of the number of differenct complaint types ####

# In[34]:


df= ser_rq_df['Complaint Type'].value_counts().rename_axis('Complaint Types').reset_index(name='Count')
...
sns.barplot(y=df['Complaint Types'],x=df['Count'])
...
# #### Types of complaints in each City in a seperate Dataset ####
...
for i in ser_rq_df['Complaint Type'].unique():
    print('Average response time for','\033[1m' + i + '\033[0m', 'is', ser_rq_df.groupby('Complaint Type')['Response Time'].get_group(i).mean())
...
while n<=50:
    for i in ser_rq_df['City'].unique():
        ...
        plt.subplot(50,1,n)
        ser_rq_df.groupby('City')['Complaint Type'].get_group(i).value_counts().head().plot(kind='pie', autopct='%1.1f%%')
        plt.axis('equal')
        plt.title(i)
        n=n+1
